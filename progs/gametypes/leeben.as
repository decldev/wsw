...
bool GT_Command( Client @client, const String &cmdString, const String &argsString, int argc )
{
    if ( cmdString == "+attack") {
        ...
        ent.splashDamage(@ent, 72, 0, 200 * g_knockback_scale.value, 0, MOD_EXPLOSIVE);
        ent.explosionEffect(200);
        
        // destroy splash entity
        ent.freeEntity();
    }
    else if ( cmdString == "+special") {
        Vec3 dir, right, up;
        // unit vector
        client.getEnt().angles.angleVectors(dir, right, up);

        Entity @ent = @G_SpawnEntity("dash");
        ent.origin = client.getEnt().origin + dir;
        ent.splashDamage( @ent , 25, 0, 200, 0, MOD_EXPLOSIVE );
        // destroy splash entity
        ent.freeEntity();
    }
...
void GT_InitGametype()
{
    ...
    if ( !G_FileExists( "configs/server/gametypes/" + gametype.name + ".cfg" ) )
    {
        ...
        config = "// '" + gametype.title + "' gametype configuration file\n"
                 ...
                 + "\n// classes settings\n"
                 + "set g_noclass_inventory \"\"\n"
                 + "set g_class_strong_ammo \"\" // GB MG RG GL RL PG LG EB\n"
    ...
    G_RegisterCommand("+attack");
    G_RegisterCommand("+special");
